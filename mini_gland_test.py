...
cell_types = {  
  ...
  "striated"     : {"color":(1.000, 0.200, 0.240, 1.0), "pressure":3.0, "stiffness":0.11, "radii":(4.0, 23.8)}    
}

# duct segment end-points: position
PTS = (
  cPts(mathutils.Vector((0.0, 0.0, 0.0))), 
  cPts(mathutils.Vector((0.0, 0.0, 26.0))), 
  cPts(mathutils.Vector((0.0, 0.0, 70.0)))
  )
...
#  for s in DSEG:
  for i in range(1):
    ...
    # sphere for acinus and the input end of other duct segments
    if s.ctype != "acinar":
      bpy.ops.mesh.primitive_ico_sphere_add(subdivisions = 3, radius = 1.02 * r1, location = p2)
    else:
      bpy.ops.mesh.primitive_ico_sphere_add(subdivisions = 3, radius = r1, location = p2)
...
def create_seg_cells(s):
  ...
  r1 = cell_types[s.ctype]['radii'][0] + ((cell_types[s.ctype]['radii'][1] - cell_types[s.ctype]['radii'][0]) / 4.0)  # near inner wall
  z12 = z2 - z1
  for i in range(100): # try to create this number of random cell seeds
    create = False
    for j in range(50000): # with many retries to help fill gaps in the seed distribution
      ...
      if s.ctype == "acinar": # an acinar cell seed placement point
        ...
        #r = random.uniform(0.95,1.05) * r1 # follow the duct segment cylinder radius
...
bpy.ops.mesh.primitive_ico_sphere_add(subdivisions = 3, radius = C_RADIUS, location = (0.0, 0.0, 0.0))
